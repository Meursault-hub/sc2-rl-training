...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
            ...
            if self.mixer is not None:
                # === START: Modified for Qatten Support ===
                mix_outs = self.mixer(chosen_action_qvals, batch["state"][:, :-1])
                attend_mag_regs = 0
                if isinstance(mix_outs, tuple):
                    chosen_action_qvals = mix_outs[0]
                    attend_mag_regs = mix_outs[1]
                else:
                    chosen_action_qvals = mix_outs
                
                target_mix_outs = self.target_mixer(target_max_qvals, batch["state"][:, 1:])
                if isinstance(target_mix_outs, tuple):
                    target_max_qvals = target_mix_outs[0]
                else:
                    target_max_qvals = target_mix_outs
                # === END: Modified for Qatten Support ===
            ...
            # === START: Add Qatten Regularization to Loss ===
            if attend_mag_regs != 0:
                loss += attend_mag_regs
            # === END ===
