...
class QattenLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if args.mixer is not None:
            ...
            elif args.mixer == "qatten_new":
                self.mixer = QattenMixer_New(args)
            else:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, show_demo=False, save_data=None):
        ...
        if self.mixer is not None:
            if self.mixer.name in ['qatten', 'qatten_new']:
        ...
        # === [修改开始] N-step Return 实现 ===
        # 获取 n_step 参数，默认为 1
        n_step = getattr(self.args, "q_nstep", 1)
        
        if n_step == 1:
            # 标准的 1-step Q-Learning
            targets = rewards + self.args.gamma * (1 - terminated) * target_max_qvals
        else:
            # N-step Q-Learning 计算逻辑
            # 1. 创建容器
            n_rewards = th.zeros_like(rewards)
            # 2. 预计算 Gamma 的幂次方 [gamma^0, gamma^1, ..., gamma^(n-1)]
            gamma_tensor = th.tensor([self.args.gamma**i for i in range(n_step)], dtype=th.float, device=n_rewards.device)
            
            # 3. 计算每个时间步往后看多少步是有效的（处理 Episode 结束边界）
            # steps 表示从当前 t 开始，往后累积多少步还在同一个 episode 内
            steps = mask.flip(1).cumsum(dim=1).flip(1).clamp_max(n_step).long()
            
            # 4. 滚动计算累积奖励 (Discounted Sum of Rewards)
            for i in range(batch.max_seq_length - 1):
                # 截取从 i 到 i+N 的 reward，与 gamma_tensor 做点积
                curr_rewards = (rewards * mask)[:, i:i+n_step, 0]
                # 注意处理边界，gamma_tensor 需要切片以匹配剩余长度
                curr_gamma = gamma_tensor[:curr_rewards.shape[1]]
                n_rewards[:, i, 0] = (curr_rewards * curr_gamma).sum(dim=1)
            
            # 5. 计算 Bootstrap 项 (Gamma^N * Q_target)
            # 找到 N 步之后的时间索引
            indices = th.linspace(0, batch.max_seq_length-2, steps=batch.max_seq_length-1, device=steps.device).unsqueeze(1).long()
            # 获取 N 步之后的 Target Q 值
            # logic: target_index = current_index + steps
            target_indices = steps.long() + indices - 1
            # 限制索引不超过最大长度
            target_indices = target_indices.clamp(max=target_max_qvals.shape[1]-1)
            
            n_targets_terminated = th.gather(target_max_qvals * (1 - terminated), dim=1, index=target_indices)
            
            # 6. 最终公式: G_t = (r + ... + g^(n-1)r) + g^n * Q(t+n)
